...
{
	public class RightmoveDownloadService : IRightmoveDownloadService
	{
		private readonly IHttpClientFactory httpClientFactory;

		public RightmoveDownloadService(IHttpClientFactory httpClientFactory)
		{
			this.httpClientFactory = httpClientFactory;
		}
		public void Download(string postcode, decimal radius, int minBedrooms, int maxBedrooms, int minPrice, int maxPrice)
		{
			string url = $"http://www.rightmove.co.uk/api/_search?locationIdentifier=POSTCODE%5E1274909&minBedrooms={minBedrooms}&minPrice={minPrice}&maxPrice={maxPrice}&numberOfPropertiesPerPage=48&radius={radius}&sortType=6&includeLetAgreed=false&viewType=LIST&dontShow=retirement%2ChouseShare&channel=RENT&areaSizeUnit=sqm&currencyCode=GBP&isFetching=false&index=";
			...
			while (true)
			{
				...
				var result = client.GetAsync(url + pageIndex).Result.Content.ReadAsAsync<SearchResult>().Result;


				foreach (var newProperty in result.properties.Where(p => p.featuredProperty == false))
				{
					if (properties.Any(p => p.id == newProperty.id)) continue;
		...
		public class Propertiesperpageoption
		{
		...
		public class Furnishoption
		{
			public string value { get; set; }
			public string description { get; set; }
		}

		public class Lettypeoption
		{
			public string value { get; set; }
			public string description { get; set; }
		}
